...
public class CreateDeliverable extends AbstractNavigatorHandler {
	...
	protected boolean nullSelectionContinue(IWorkbenchPart part,
		...
		MessageUtil.showToast(Messages.get().CreateDeliverable_0,
				SWT.ICON_WARNING);
	...
	protected void execute(PrimaryObject selected, IWorkbenchPart part,
		...
		try {
			if (currentViewerControl != null) {
				...
				// 使用编辑器打开编辑工作定义
				Configurator conf = Widgets.getEditorRegistry()
						.getConfigurator(po.getDefaultEditorId());
				DataObjectDialog.openDialog(po, (DataEditorConfigurator) conf,
						true, null,
						Messages.get().CreateDeliverable_1 + po.getTypeName());
				// 4. 将更改消息传递到编辑器
			...
			e.printStackTrace();
			MessageUtil.showToast(shell, Messages.get().CreateDeliverable_2
					+ po.getTypeName(), e.getMessage(), SWT.ICON_ERROR);
		}
