...
public class BudgetItem extends PrimaryObject {
	...
	public static BudgetItem GET_DEFAULT_BUDGET_ITEM(){
		...
		DBObject data = col.findOne(new BasicDBObject().append(F_ISDEFAULT, Boolean.TRUE));
		if(data==null){
			data = new BasicDBObject().append(F_ISDEFAULT, Boolean.TRUE).append(F__ID, new ObjectId()).append(F_DESC, "ƒ¨»œ‘§À„");
			col.insert(data);
		}
	...
	public BudgetItem[] getChildren() {
		 ...
		 if(childrenData!=null){
			 ...
			 for(int i=0;i<childrenData.size();i++){
				 result[i]=ModelService.createModelObject((DBObject)childrenData.get(i), BudgetItem.class);
	...
	public void createChild(String budgetItemName) {
		 ...
		 childrenData.add(new BasicDBObject().append(F_DESC, budgetItemName).append(F__ID, new ObjectId()));
