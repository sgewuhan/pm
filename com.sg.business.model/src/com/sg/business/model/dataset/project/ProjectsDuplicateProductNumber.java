...
public class ProjectsDuplicateProductNumber extends
	...
	public DataSet getDataSet() {
		...
		while (cur.hasNext()) {
			...
			DBCursor c = col.find(new BasicDBObject().append(
					ProductItem.F_DESC, pn));
			if (c.size() > 1) {
				while (c.hasNext()) {
					DBObject d = c.next();
		...
		DBCollection pjcol = DBActivator.getCollection(IModelConstants.DB,
				IModelConstants.C_PROJECT);
		DBCursor c1 = pjcol.find(new BasicDBObject().append(Project.F__ID,
				new BasicDBObject().append("$in", projectIds.toArray())));
