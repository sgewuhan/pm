...
import com.sg.business.finance.eai.BasicPeriodCostAdapter;
...
public class WorkorderCostGather implements ISchedualJobRunnable {
	...
	public boolean run() throws Exception {
		BasicPeriodCostAdapter adapter = new BasicPeriodCostAdapter();
		...
		for (int i = 0; i > -23; i--) {
			...
			try {
				Commons.LOGGER.info("[成本数据]准备获取SAP工作令号研发成本数据:" + year + "-" + month);
				adapter.runGetData(null, workOrders, costElementArray, year,
						month, null,IModelConstants.C_WORKORDER_COST);
			} catch (Exception e) {
				Commons.LOGGER.error("[成本数据]获得SAP工作令号研发成本失败:" + year + "-" + month, e);
				throw e;
			}
