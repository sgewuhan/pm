...
public class ParaXLabel extends ColumnLabelProvider {
	...
	public String getText(Object element) {
		 ...
		 * 根据typeId获取工时类型名，根据programId获取工时方案名称 ObjectId typeId = new
		 * ObjectId(typeIdAndProgramId[0]); ObjectId programId = new
		 * ObjectId(typeIdAndProgramId[1]);
		 */
		DBObject typeIdAndProgramId = (DBObject) element;
		ObjectId typeId = (ObjectId) typeIdAndProgramId
				.get(IWorkCloneFields.F_WORKTIME_PARAX_ID);
		ObjectId programId = (ObjectId) typeIdAndProgramId
		...
		for (int i = 0; i < paraXs.size(); i++) {
			ObjectId paraXId = (ObjectId) ((DBObject) paraXs.get(i))
					.get(WorkTimeProgram.F__ID);
			if (paraXId.equals(typeId)) {
				String paraXDesc = (String) ((DBObject) paraXs.get(i))
